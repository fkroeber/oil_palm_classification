...
class NN:
    ...
    def __init__(self, **kwargs):
        """
        Models can be retrieved with/without pre-trained weights & with the last layer being adjusted to the number of required classes.
        model_name: name of the model to be used - available args are "densenet", "inception", "max_vit", "resnet", "resnext", "senet", "squeezenet", "swin_vit", "vit", "wideresnet"
        mode: degree of use of pretrained models - available args are "scratch", "finetune", "feat_extract"
    ...
    def init_model(self):
        ...
        For each model the model object along with necessary transforms to make the use of pretrained weights applicable is returned.
        Note: The data transforms are pretraining specific but in most cases for Imagenet pretraining something like
        ...
        if self.model_name == "densenet":
            ...
            w_apply = w if self.use_pretrained else None
            model = models.inception_v3(weights=w_apply)
            self._set_parameter_requires_grad(model, self.feature_extract)
